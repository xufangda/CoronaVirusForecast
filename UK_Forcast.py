...
k0 = 0.01       # quarantine rate for exposed
...
nmax = days*interval+1
tt = np.linspace(0,days,nmax) #time
...
# #plot the quarantine and hospitalizations
# heal, = ax.plot(tt, yy[:,2]+yy[:,5],c='peru', lw=2) 
...
# detected data
detected=yy[:,2]+yy[:,3]+yy[:,4]+yy[:,5]
for i in range(0,days+10,10):
    tool_tip='{0}\n{1}'.format('Day'+str(i), int(detected[i*interval]))
